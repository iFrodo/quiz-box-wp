...
function bazz_widget_send()
{
	...
	$message = trim(strip_tags("Перезвонить клиенту :%0A" . '%2B'. $phone . '%0A' . 'Имя: ' . $name . ' ' . '%0A' . $blog_url .'%0A'. $_POST['refferer']));
	...
	$ch = curl_init('https://api.telegram.org/bot' . TELEGRAM_TOKEN . '/sendContact?chat_id=' . TELEGRAM_CHATID . '&phone_number=' . $phone. '&first_name=' . $name )   ; // URL
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, true); // Не возвращать ответ
	curl_exec($ch); // Делаем запрос
	curl_close($ch); // Завершаем сеанс cURL
	// Protection
...
{ ?>
	...
	<style>
		...
		<?php if (is_rtl()) : ?>.bazz-widget-form-top,
...
{ ?>
	...
	<form method="post" action="options.php" id="bazz-settings-form">
		...
		<div class="option-timer">
			<label for=""><?php _e('The countdown will be started on', 'bazz-callback-widget'); ?> <input type="text" name="bazz_options[time]" value="<?php echo esc_attr($bazz_options['time']); ?>" /> <?php _e('seconds.', 'bazz-callback-widget'); ?>
			</label>
